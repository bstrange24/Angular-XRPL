...
export class SendChecksComponent implements AfterViewChecked {
     ...
     async cashCheck() {
          ...
          const errors = this.validateInputs(inputs, 'cash');
          if (errors.length > 0) {
               return this.setError(`ERROR: ${errors.join('; ')}`);
          }
     ...
     private validateInputs(inputs: ValidationInputs, action: string): string[] {
          ...
          const actionConfig: Record<string, { required: (keyof ValidationInputs)[]; customValidators?: (() => string | null)[] }> = {
               ...
               cash: {
                    ...
                    customValidators: [() => isValidSeed(inputs.seed), () => isValidXrpAddress(inputs.destination, 'Destination'), () => isValidNumber(inputs.amount, 'Amount', 0), () => isRequired(inputs.checkId, 'Check ID')],
               },
               cancel: {
